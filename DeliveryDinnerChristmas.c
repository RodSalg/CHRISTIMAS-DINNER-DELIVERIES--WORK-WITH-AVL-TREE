...
no *insere(no *raiz, int novo, int regiao, int minuto, int pedido)
{
    ...
    {
        // printf("\n%d e %d\n", novo, raiz->chave);
        if ((novo * 60 + minuto) < raiz->chave)
        { // se for menor eu insiro do lado esquerto
        ...
        {
            if ((novo * 60 + minuto) > raiz->chave)
            { // se for maior eu insiro do lado direito
...
no *remover(no *raiz, int chave)
{
    ...
    else
    {
        ...
        if (chave < raiz->chave)
        {
            ...
            // printf("\n if chave: %d < raiz->chave: %d", chave, raiz->chave);
        ...
        else
        {
            ...
            if (chave > raiz->chave)
            {
                ...
                // printf("\n else chave: %d > raiz->chave: %d", chave, raiz->chave);
...
// funcao para escolher o tipo de comida
...
no *transfereArvore(no *arvore, no *novaArvore)
{
...
tipoEntregador *formataEntregadores(int numEntregadores)
{
    ...
    // mostrar entregadores disponiveis
    /*    for(int i = 0; i < numEntregadores; i++){
             printf("\n entregador %d: \n", i);
                 // 0 - está livre
                 // 1 - esta ocupado
             for(int j = 0; j < 8; j++){
                 ...
                 printf(" %d  ", entregadores[i].entregador[j]);
...
no *Comanda(no *ArvEntregas, no *arvore, tipoEntregador *entregadores, int numEntregadores, int relogio)
{
    ...
    for (int i = 0; i < numEntregadores; i++)
    {
        for (int j = 0; j < 8; j++)
        {
            ...
            { // agora sim eu vejo se o entregador está livre
                // printf("valor a ser removido %d\n ",entregadores[i].entregas[j]->chave );
                ArvEntregas = insere(ArvEntregas, entregadores[i].entregas[j]->chaveHora, entregadores[i].entregas[j]->regiao,
                    entregadores[i].entregas[j]->chaveMinuto, entregadores[i].entregas[j]->pedido); // considero o pedido entregue
    ...
    return auxVIRGEM;
    // como fazer para retornar os dois ao mesmo tempo???
}
        ...
        for (int i = 0; i < numEntregadores; i++)
        {
            ...
            for (int j = 0; j < 8; j++)
            {
                ...
                if (entregadores[i].entregador[j] == 0 && (raiz->chave <= relogio + 59))
                {
                    ...
                    achei = 1;
                    entregadores[0].contagemPedidos++;
                    break;
                }
            }
            if (achei == 1)
...
void imprimirTodasAsEntregas(no *arvore)
{
    if (arvore)
    {
        ...
        printf("\n\tPedido %d, entregue as: %dH%dMIN, e sua opcao de ceia foi: %d\n", arvore->chave, 
                                            arvore->chaveHora, arvore->chaveMinuto, arvore->pedido);
        imprimirTodasAsEntregas(arvore->dir);
    }
}

void entregasDeNatal(){
    //    printf("%s", definirRegiao(2));
    ...
    //int  = 32;
    // tratando a parte do tempo agora
    int relogio = 8 * 60;
    while (relogio <= 22 * 60)
    {
